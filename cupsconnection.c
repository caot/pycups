...
void set_ipp_error(ipp_status_t status, const char *message) {
  ...
  PyObject *v = Py_BuildValue("(iu)", status,
      PyUnicode_AS_UNICODE(PyUnicode_FromString(message)));
#endif
  if (v != NULL) {
...
static void construct_uri(char *buffer, size_t buflen, const char *base,
    const char *value) {
  char *d = buffer;
...
static int Connection_init(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|sii", kwlist, &host, &port,
      &encryption))
    return -1;
...
static void Connection_dealloc(Connection *self) {
  ...
  if (j < NumConnections) {
    if (NumConnections > 1) {
      Connection **new_array = calloc(NumConnections - 1, sizeof(Connection*));
...
Connection_repr(Connection *self) {
  char buffer[256];
  snprintf(buffer, 256, "<cups.Connection object for %s at %p>", self->host,
      self);
#if PY_MAJOR_VERSION >= 3
...
#ifdef HAVE_CUPS_1_4
static const char*
password_callback(int newstyle, const char *prompt, http_t *http,
    const char *method, const char *resource, void *user_data) {
  struct TLS *tls = get_TLS();
  ...
  free(self->cb_password);
  if (result == Py_None || !UTF8_from_PyObj(&self->cb_password, result))
    self->cb_password = NULL;
...
const char*
password_callback_oldstyle(const char *prompt, http_t *http, const char *method,
    const char *resource, void *user_data) {
  return password_callback(0, prompt, http, method, resource, user_data);
}

const char*
password_callback_newstyle(const char *prompt, http_t *http, const char *method,
    const char *resource, void *user_data) {
...
do_printer_request(Connection *self, PyObject *args, PyObject *kwds,
  ...
  case CUPS_REJECT_JOBS: {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|O", kwlist, &nameobj,
        &reasonobj))
      return NULL;
    break;
  }
...
Connection_getDests(Connection *self) {
  ...
  for (i = 0; i <= num_dests; i++) {
    ...
    Dest *destobj = (Dest*) PyType_GenericNew(&cups_DestType, largs, lkwlist);
    Py_DECREF(largs);
    Py_DECREF(lkwlist);
...
int cups_dest_cb(void *user_data, unsigned flags, cups_dest_t *dest) {
  ...
  destobj = (Dest*) PyType_GenericNew(&cups_DestType, largs, lkwlist);
  Py_DECREF(largs);
  Py_DECREF(lkwlist);
  copy_dest(destobj, dest);
  args = Py_BuildValue("(OiO)", context->user_data, flags, destobj);
  Py_DECREF((PyObject*) destobj);
...
PyObject_from_attr_value(ipp_attribute_t *attr, int i) {
  ...
  case IPP_TAG_RANGE:
    ...
    val = Py_BuildValue("(ii)", lower, upper);
    break;
  ...
  case IPP_TAG_RESOLUTION:
    ...
    val = Py_BuildValue("(iii)", xres, yres, units);
    break;
  default:
    snprintf(unknown, sizeof(unknown), "(unknown IPP value tag 0x%x)",
        ippGetValueTag(attr));
...
Connection_getPrinters(Connection *self) {
  ...
  const char *attributes[] = { "printer-name", "printer-type",
      "printer-location", "printer-info", "printer-make-and-model",
  ...
  ippAddStrings(request, IPP_TAG_OPERATION, IPP_TAG_KEYWORD,
      "requested-attributes", sizeof(attributes) / sizeof(attributes[0]),
      NULL, attributes);
  ...
  result = PyDict_New();
  for (attr = ippFirstAttribute(answer); attr;
    ...
    dict = PyDict_New();
    for (; attr && ippGetGroupTag(attr) == IPP_TAG_PRINTER; attr =
        ippNextAttribute(answer)) {
      PyObject *val = NULL;
  ...
  val = PyLong_FromLong (ptype);
...
#endif
      } else if ((!strcmp(ippGetName(attr), "printer-make-and-model")
      ...
      } else if (!strcmp(ippGetName(attr), "printer-state-reasons")
...
#endif
      } else if ((!strcmp(ippGetName(attr), "printer-up-time")
          || !strcmp(ippGetName(attr), "queued-job-count"))
...
Connection_getClasses(Connection *self) {
  ...
  ipp_attribute_t *attr;
  const char *attributes[] = { "printer-name", "member-names", };
...
do_getPPDs(Connection *self, PyObject *args, PyObject *kwds, int all_lists) {
  ...
  char *ppd_psversion;
  char *ppd_type = NULL;
  static char *kwlist[] = { "limit", "exclude_schemes", "include_schemes",
      "ppd_natural_language", "ppd_device_id", "ppd_make", "ppd_make_and_model",
      "ppd_model_number", "ppd_product", "ppd_psversion", "ppd_type",
      NULL };

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iOOsOOOiOOs", kwlist, &limit,
      &exclude_schemes_obj, &include_schemes_obj, &ppd_natural_language,
      &ppd_device_id_obj, &ppd_make_obj, &ppd_make_and_model_obj,
      &ppd_model_number, &ppd_product_obj, &ppd_psversion_obj, &ppd_type))
    return NULL;

  request = ippNewRequest(CUPS_GET_PPDS);
  if (limit > 0)
    ippAddInteger(request, IPP_TAG_OPERATION, IPP_TAG_INTEGER, "limit", limit);
  ...
  if (ppd_make_obj) {
    ...
    ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_TEXT, "ppd-make", NULL,
        ppd_make);
    free(ppd_make);
  }
  ...
  if (ppd_product_obj) {
    ...
    ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_TEXT, "ppd-product", NULL,
        ppd_product);
    free(ppd_product);
  }
  ...
  if (ppd_psversion_obj) {
    ...
    ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_TEXT, "ppd-psversion",
        NULL, ppd_psversion);
    free(ppd_psversion);
  }
  ...
  if (ppd_type)
    ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_KEYWORD, "ppd-type", NULL,
        ppd_type);
...
Connection_getDocument(Connection *self, PyObject *args) {
  ...
  if ((attr = ippFindAttribute(answer, "document-format", IPP_TAG_MIMETYPE))
      != NULL)
    format = ippGetString(attr, 0, NULL);

  if ((attr = ippFindAttribute(answer, "document-name", IPP_TAG_NAME)) != NULL)
  ...
  debugprintf("<- Connection_getDocument() = {'file':\"%s\","
      ...
      format ? format : "(nul)", name ? name : "(nul)");
  ippDelete(answer);
  return dict;
...
Connection_getDevices(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iOOi", kwlist, &limit,
      &exclude_schemes, &include_schemes, &timeout))
    return NULL;
...
static int get_requested_attrs(PyObject *requested_attrs, size_t *n_attrs,
    char ***attrs) {
  int i;
  size_t n;
  char **as;
...
Connection_getJobs(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|siiiO", kwlist, &which,
      &my_jobs, &limit, &first_job_id, &requested_attrs))
    return NULL;
  ...
  ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_KEYWORD, "which-jobs", NULL,
      which ? which : "not-completed");
  ...
  if (requested_attrs) {
    ippAddStrings(request, IPP_TAG_OPERATION, IPP_TAG_KEYWORD,
        "requested-attributes", n_attrs, NULL, (const char**) attrs);
  ...
  for (attr = ippFirstAttribute(answer); attr;
    ...
    for (; attr && ippGetGroupTag(attr) == IPP_TAG_JOB;
      ...
      debugprintf("  Attr:  %s\n", ippGetName(attr));
      if (!strcmp(ippGetName(attr), "job-id") && job_id == -1
          && ippGetValueTag(attr) == IPP_TAG_INTEGER) {
...
Connection_getJobAttributes(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "i|O", kwlist, &job_id,
      &requested_attrs))
    return NULL;
...
Connection_cancelJob(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "i|i", kwlist, &job_id,
      &purge_job))
    return NULL;
...
Connection_cancelAllJobs(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OOii", kwlist, &nameobj,
      &uriobj, &my_jobs, &purge_jobs))
    return NULL;
  ...
  if (nameobj) {
    ...
    PyErr_SetString(PyExc_RuntimeError, "name or uri must be specified");
    return NULL;
  }
  ...
  if (nameobj) {
    construct_uri(consuri, sizeof(consuri), "ipp://localhost/printers/", name);
    uri = consuri;
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (answer && ippGetStatusCode(answer) == IPP_NOT_POSSIBLE) {
      ...
      construct_uri(consuri, sizeof(consuri), "ipp://localhost/classes/", name);
    } else
      break;
  }
...
Connection_createJob(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "OOO", kwlist, &printer_obj,
      &title_obj, &options_obj))
    return NULL;
  ...
  while (PyDict_Next(options_obj, &pos, &key, &val)) {
    ...
    num_settings = cupsAddOption(UTF8_from_PyObj(&name, key),
        UTF8_from_PyObj(&value, val), num_settings, &settings);
    free(name);
    free(value);
  }
...
Connection_startDocument(Connection *self, PyObject *args, PyObject *kwds) {
  static char *kwlist[] = { "printer", "job_id", "doc_name", "format",
      "last_document", NULL };
  PyObject *printer_obj;
  char *printer;
  int jobid;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "OiOOi", kwlist, &printer_obj,
      &jobid, &doc_name_obj, &format_obj, &last_document))
    return NULL;
  ...
  debugprintf(
      "-> Connection_startDocument(printer=%s, jobid=%d, doc_name=%s, format=%s)\n",
      printer, jobid, doc_name, format);

  Connection_begin_allow_threads(self);
  status = cupsStartDocument(self->http, printer, jobid, doc_name, format,
      last_document);
  Connection_end_allow_threads(self);
...
Connection_writeRequestData(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "Oi", kwlist, &buffer_obj,
      &length))
    return NULL;
...
Connection_moveJob(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OiO", kwlist, &printeruriobj,
      &job_id, &jobprinteruriobj))
    return NULL;
  ...
  if (printeruriobj) {
    ...
    PyErr_SetString(PyExc_RuntimeError, "job_id or printer_uri required");
    return NULL;
  }
...
Connection_setJobHoldUntil(Connection *self, PyObject *args) {
  ...
  ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_NAME, "requesting-user-name",
      NULL, cupsUser());
  num_options = cupsAddOption("job-hold-until", job_hold_until, num_options,
      &options);
...
Connection_restartJob(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  char uri[1024];
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "i|s", kwlist, &job_id,
      &job_hold_until))
    return NULL;
  ...
  ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_NAME, "requesting-user-name",
      NULL, cupsUser());
  if (job_hold_until)
    ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_NAME, "job-hold-until",
        NULL, job_hold_until);
...
Connection_getFile(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|siO", kwlist, &resource,
      &filename, &fd, &fileobj))
    return NULL;

  if ((fd > -1 && (filename || fileobj)) || (filename && fileobj)) {
    PyErr_SetString(PyExc_RuntimeError,
...
Connection_addPrinter(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = { "name", "filename", "ppdname", "info", "location",
      "device", "ppd", NULL };

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OOOOOO", kwlist, &nameobj,
      &ppdfileobj, &ppdnameobj, &infoobj, &locationobj, &deviceobj, &ppd))
    return NULL;
  ...
  debugprintf("-> Connection_addPrinter(%s,%s,%s,%s,%s,%s,%s)\n", name,
      ppdfile ? ppdfile : "", ppdname ? ppdname : "", info ? info : "",
  ...
  if (ppds_specified > 1) {
    PyErr_SetString(PyExc_RuntimeError, "Only one PPD may be given");
    debugprintf("<- Connection_addPrinter() EXCEPTION\n");
    free(name);
    free(ppdfile);
    free(ppdname);
...
Connection_setPrinterErrorPolicy(Connection *self, PyObject *args) {
  ...
  request = add_modify_printer_request(name);
  for (i = 0; i < 2; i++) {
    ippAddString(request, IPP_TAG_PRINTER, IPP_TAG_NAME, "printer-error-policy",
        NULL, policy);
    Connection_begin_allow_threads(self);
    answer = cupsDoRequest(self->http, request, "/admin/");
    Connection_end_allow_threads(self);
...
Connection_setPrinterOpPolicy(Connection *self, PyObject *args) {
  ...
  request = add_modify_printer_request(name);
  for (i = 0; i < 2; i++) {
    ippAddString(request, IPP_TAG_PRINTER, IPP_TAG_NAME, "printer-op-policy",
        NULL, policy);
    Connection_begin_allow_threads(self);
    answer = cupsDoRequest(self->http, request, "/admin/");
    Connection_end_allow_threads(self);
...
do_requesting_user_names(Connection *self, PyObject *args,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (num_users) {
      attr = ippAddStrings(request, IPP_TAG_PRINTER, IPP_TAG_NAME, requeststr,
          num_users, NULL, NULL);
...
Connection_addPrinterOptionDefault(Connection *self, PyObject *args) {
  ...
  for (i = 0; i < 2; i++) {
    if (!PyUnicode_Check(pyvalue) && !PyBytes_Check(pyvalue)
      ...
      int j;
      attr = ippAddStrings(request, IPP_TAG_PRINTER, IPP_TAG_NAME, opt, len,
          NULL, NULL);
      for (j = 0; j < len; j++) {
      ...
      ippAddString(request, IPP_TAG_PRINTER, IPP_TAG_NAME, opt, NULL,
          PyObject_to_string(pyvalue));
...
Connection_deletePrinterOptionDefault(Connection *self, PyObject *args) {
  ...
  request = add_modify_printer_request(name);
  for (i = 0; i < 2; i++) {
    ippAddString(request, IPP_TAG_PRINTER, IPP_TAG_DELETEATTR, opt, NULL, NULL);
    Connection_begin_allow_threads(self);
    answer = cupsDoRequest(self->http, request, "/admin/");
    Connection_end_allow_threads(self);
    if (PyErr_Occurred()) {
...
Connection_getPrinterAttributes(Connection *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OOO", kwlist, &nameobj,
      &uriobj, &requested_attrs))
    return NULL;
  ...
  for (attr = ippFirstAttribute(answer); attr;
    ...
    for (; attr && ippGetGroupTag(attr) == IPP_TAG_PRINTER; attr =
      ...
      if (!is_list && namelen > 10) {
        const char *multivalue_options[] = { "notify-events-default",
            "requesting-user-name-allowed", "requesting-user-name-denied",
            "printer-state-reasons", "marker-colors", "marker-names",
            "marker-types", "marker-levels", "member-names",
            NULL };
...
Connection_addPrinterToClass(Connection *self, PyObject *args) {
  ...
  request = ippNewRequest(IPP_GET_PRINTER_ATTRIBUTES);
  construct_uri(classuri, sizeof(classuri), "ipp://localhost/classes/",
      classname);
  ...
  ippAddString(request, IPP_TAG_OPERATION, IPP_TAG_URI, "printer-uri", NULL,
      classuri);
  construct_uri(printeruri, sizeof(printeruri), "ipp://localhost/printers/",
      printername);
  free(printername);
  if (answer) {
    ipp_attribute_t *printers;
...
Connection_deletePrinterFromClass(Connection *self, PyObject *args) {
  const char *requested_attrs[] = { "member-names", "member-uris" };
  PyObject *printernameobj;
  char *printername;
  PyObject *classnameobj;
  char *classname;
...
Connection_getPPD3(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OO", kwlist, &printerobj,
      &fmodtime, &filenameobj))
    return NULL;
  ...
  Connection_begin_allow_threads(self);
  status = cupsGetPPD3(self->http, printer, &modtime, fname, sizeof(fname));
  Connection_end_allow_threads(self);
...
Connection_printTestPage(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OOOO", kwlist, &printerobj,
      &fileobj, &titleobj, &formatobj, &userobj))
    return NULL;
  ...
  if (!fileobj) {
    const char *testprint[] = { "%s/data/testprint", "%s/data/testprint.ps",
    NULL };
    if ((datadir = getenv("CUPS_DATADIR")) != NULL) {
      ...
      const char *const dirs[] = { "/usr/share/cups", "/usr/local/share/cups",
      NULL };
      int found = 0;
      int i;
  ...
  construct_uri(uri, sizeof(uri), "ipp://localhost/printers/", printer);
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (answer && ippGetStatusCode(answer) == IPP_NOT_POSSIBLE) {
      ippDelete(answer);
      // Perhaps it's a class, not a printer.
      construct_uri(uri, sizeof(uri), "ipp://localhost/classes/", printer);
    } else
      break;
  }
...
Connection_adminExportSamba(Connection *self, PyObject *args) {
  ...
  if (!cupsAdminCreateWindowsPPD(self->http, name, ppdfile, sizeof(ppdfile))) {
    PyErr_SetString(PyExc_RuntimeError, "No PPD file found for the printer");
    return NULL;
  }
...
Connection_adminSetServerSettings(Connection *self, PyObject *args) {
  ...
  while (PyDict_Next(dict, &pos, &key, &val)) {
    ...
    debugprintf("%s: %s\n", name, value);
    num_settings = cupsAddOption(name, value, num_settings, &settings);
    free(name);
    free(value);
  }
...
Connection_getSubscriptions(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|Oi", kwlist, &uriobj,
      &my_subscriptions, &job_id))
    return NULL;
...
static PyObject*
Connection_createSubscription(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OiOiiO", kwlist,
      &resource_uriobj, &events, &job_id, &recipient_uriobj, &lease_duration,
  ...
  ippAddString(request, IPP_TAG_SUBSCRIPTION, IPP_TAG_CHARSET, "notify-charset",
      NULL, "utf-8");
  ...
  if (events) {
    attr = ippAddStrings(request, IPP_TAG_SUBSCRIPTION, IPP_TAG_KEYWORD,
        "notify-events", n, NULL, NULL);
    for (i = 0; i < n; i++) {
      PyObject *event = PyList_GetItem(events, i);
...
Connection_getNotifications(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|O", kwlist, &subscription_ids,
      &sequence_numbers))
    return NULL;
...
Connection_renewSubscription(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "i|i", kwlist, &id,
      &lease_duration))
    return NULL;
...
Connection_printFile(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "OOOO", kwlist, &printer_obj,
      &filename_obj, &title_obj, &options_obj))
    return NULL;
...
static void free_string_list(int num_string, char **strings) {
  int i;
...
Connection_printFiles(Connection *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "OOOO", kwlist, &printer_obj,
      &filenames_obj, &title_obj, &options_obj))
    return NULL;
  ...
  while (PyDict_Next(options_obj, &pos, &key, &val)) {
    ...
    num_settings = cupsAddOption(UTF8_from_PyObj(&name, key),
        UTF8_from_PyObj(&value, key), num_settings, &settings);
    free(name);
    free(value);
  }
  ...
  jobid = cupsPrintFiles2(self->http, printer, num_filenames,
      (const char**) filenames, title, num_settings, settings);
  Connection_end_allow_threads(self);
...
PyMethodDef Connection_methods[] =
    { { "getPrinters", (PyCFunction) Connection_getPrinters, METH_NOARGS,
        "getPrinters() -> dict\n\n"
            "@return: a dict, indexed by name, of dicts representing\n"
            "queues, indexed by attribute.\n"
            "@raise IPPError: IPP problem" },

        { "getDests", (PyCFunction) Connection_getDests, METH_NOARGS,
            "getDests() -> dict\n\n"
        ...
        { "getClasses", (PyCFunction) Connection_getClasses, METH_NOARGS,
            "getClasses() -> dict\n\n"
        ...
        { "getPPDs", (PyCFunction) Connection_getPPDs, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getPPDs2", (PyCFunction) Connection_getPPDs2, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getServerPPD", (PyCFunction) Connection_getServerPPD, METH_VARARGS,
        ...
        { "getDocument", (PyCFunction) Connection_getDocument, METH_VARARGS,
        ...
        { "getDevices", (PyCFunction) Connection_getDevices, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getJobs", (PyCFunction) Connection_getJobs, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getJobAttributes", (PyCFunction) Connection_getJobAttributes,
        ...
        { "cancelJob", (PyCFunction) Connection_cancelJob, METH_VARARGS
            | METH_KEYWORDS, "cancelJob(jobid, purge_job=False) -> None\n\n"
            "@type jobid: integer\n"
        ...
        { "cancelAllJobs", (PyCFunction) Connection_cancelAllJobs, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "moveJob", (PyCFunction) Connection_moveJob, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "authenticateJob", (PyCFunction) Connection_authenticateJob,
            METH_VARARGS, "authenticateJob(jobid, auth_info=None) -> None\n\n"
                "@type jobid: integer\n"
        ...
        { "setJobHoldUntil", (PyCFunction) Connection_setJobHoldUntil,
            METH_VARARGS,
        ...
        { "restartJob", (PyCFunction) Connection_restartJob, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getFile", (PyCFunction) Connection_getFile, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "putFile", (PyCFunction) Connection_putFile, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "addPrinter", (PyCFunction) Connection_addPrinter, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "setPrinterDevice", (PyCFunction) Connection_setPrinterDevice,
            METH_VARARGS, "setPrinterDevice(name, device_uri) -> None\n\n"
        ...
        { "setPrinterInfo", (PyCFunction) Connection_setPrinterInfo,
            METH_VARARGS, "setPrinterInfo(name, info) -> None\n\n"
        ...
        { "setPrinterLocation", (PyCFunction) Connection_setPrinterLocation,
            METH_VARARGS, "setPrinterLocation(name, location) -> None\n\n"
        ...
        { "setPrinterShared", (PyCFunction) Connection_setPrinterShared,
            METH_VARARGS,
        ...
        { "setPrinterJobSheets", (PyCFunction) Connection_setPrinterJobSheets,
            METH_VARARGS, "setPrinterJobSheets(name, start, end) -> None\n\n"
        ...
        { "setPrinterOpPolicy", (PyCFunction) Connection_setPrinterOpPolicy,
            METH_VARARGS,
        ...
        { "deletePrinter", (PyCFunction) Connection_deletePrinter, METH_VARARGS
            | METH_KEYWORDS, "deletePrinter(name) -> None\n\n"
            "Delete a printer.\n\n"
            "@type name: string\n"
            "@param name: queue name\n"
            "@raise IPPError: IPP problem" },

        { "getPrinterAttributes", (PyCFunction) Connection_getPrinterAttributes,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "addPrinterToClass", (PyCFunction) Connection_addPrinterToClass,
            METH_VARARGS, "addPrinterToClass(name, class) -> None\n\n"
        ...
        { "deleteClass", (PyCFunction) Connection_deleteClass, METH_VARARGS,
        ...
        { "getDefault", (PyCFunction) Connection_getDefault, METH_NOARGS,
            "getDefault() -> string or None\n\n"
                "Get the system default printer.\n\n"
                "@return: default printer name or None" },

        { "setDefault", (PyCFunction) Connection_setDefault, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "getPPD", (PyCFunction) Connection_getPPD, METH_VARARGS,
            "getPPD(name) -> string\n\n"
...
#ifdef HAVE_CUPS_1_4
        { "getPPD3", (PyCFunction) Connection_getPPD3, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "enablePrinter", (PyCFunction) Connection_enablePrinter, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "disablePrinter", (PyCFunction) Connection_disablePrinter,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "acceptJobs", (PyCFunction) Connection_acceptJobs, METH_VARARGS
            | METH_KEYWORDS, "acceptJobs(name) -> None\n\n"
            "Cause printer to accept jobs.\n"
            "@type name: string\n"
            "@param name: queue name\n"
            "@raise IPPError: IPP problem" },

        { "rejectJobs", (PyCFunction) Connection_rejectJobs, METH_VARARGS
            | METH_KEYWORDS,
            "rejectJobs(name)\n\n"
        ...
        { "printTestPage", (PyCFunction) Connection_printTestPage, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "adminExportSamba", (PyCFunction) Connection_adminExportSamba,
            METH_VARARGS,
        ...
        { "getSubscriptions", (PyCFunction) Connection_getSubscriptions,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "createSubscription", (PyCFunction) Connection_createSubscription,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "getNotifications", (PyCFunction) Connection_getNotifications,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "cancelSubscription", (PyCFunction) Connection_cancelSubscription,
            METH_VARARGS, "cancelSubscription(id) -> None\n\n"
                "Cancel a subscription.\n\n"
                "@type id: integer\n"
                "@param id: subscription ID\n"
                "@raise IPPError: IPP problem" },

        { "renewSubscription", (PyCFunction) Connection_renewSubscription,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "createJob", (PyCFunction) Connection_createJob, METH_VARARGS
            | METH_KEYWORDS, "createJob(printer, title, options) -> integer\n\n"
        ...
        { "startDocument", (PyCFunction) Connection_startDocument, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "writeRequestData", (PyCFunction) Connection_writeRequestData,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "finishDocument", (PyCFunction) Connection_finishDocument,
            METH_VARARGS | METH_KEYWORDS,
        ...
        { "printFile", (PyCFunction) Connection_printFile, METH_VARARGS
            | METH_KEYWORDS,
        ...
        { "printFiles", (PyCFunction) Connection_printFiles, METH_VARARGS
            | METH_KEYWORDS,
...
static int Dest_init(Dest *self, PyObject *args, PyObject *kwds) {
  self->num_options = 0;
  return 0;
}

static void Dest_dealloc(Dest *self) {
  if (self->num_options) {
    int i;
...
Dest_repr(Dest *self) {
  char buffer[256];
  snprintf(buffer, 256, "<cups.Dest %s%s%s%s>", self->destname,
      self->instance ? "/" : "", self->instance ? self->instance : "",
...
PyGetSetDef Dest_getseters[] = { { "name", (getter) Dest_getName, (setter) NULL,
    "name", NULL },

{ "instance", (getter) Dest_getInstance, (setter) NULL, "instance", NULL },

{ "is_default", (getter) Dest_getIsDefault, (setter) NULL, "is_default", NULL },

{ "options", (getter) Dest_getOptions, (setter) NULL, "options", NULL },
